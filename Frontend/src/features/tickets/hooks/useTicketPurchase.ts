...
export function useTicketPurchase() {
  ...
  const getTicketResumes = useCallback(async (): Promise<TicketResume[]> => {
    setState({ loading: true, error: null });
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to fetch ticket resumes";
      setState({ loading: false, error: errorMessage });
      throw err;
    } finally {
      setState((prev) => ({ ...prev, loading: false }));
  ...
  const getQrImage = useCallback(
    async (purchaseId: number): Promise<string> => {
      try {
        const blob = await ticketsApi.getPurchaseQr(purchaseId);
        return URL.createObjectURL(blob);
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Failed to fetch QR";
